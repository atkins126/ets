...
struct CEncodingUnit {
    int m_iCodePage;
    char *m_sAliasList;
};

static CEncodingUnit g_stEncodings[] = {
...
bool IsInList(const char *p_sToken, const char *p_sList)
{
    if ((!p_sToken) || (!p_sList))
    {
        return false;
    ...
    vcl4c::string::SplitString(vList, p_sList, ' ');

    for (auto it = vList.begin(); it != vList.end(); ++it)
    {
        if (0 == _stricmp(it->c_str(), p_sToken))
        {
            return true;
        }
    }
...
int GetEncodingFromString(const char *p_sCodePage)
{
    if (IsInList(p_sCodePage, "utf-8 utf8"))
    {
        return 65001;
    }

    for (size_t i = 0; i < sizeof(g_stEncodings) / sizeof(CEncodingUnit); ++i)
    {
        if (IsInList(p_sCodePage, g_stEncodings[i].m_sAliasList))
        {
            return g_stEncodings[i].m_iCodePage;
        }
    }
...
extern "C" int __stdcall DetectCodePage(char *p_sValue, size_t p_iLen)
{
    uchardet_t ud = uchardet_new();
    uchardet_handle_data(ud, p_sValue, p_iLen);
    uchardet_data_end(ud);
    const char* sCodePage = uchardet_get_charset(ud);

    int iCodePage = GetEncodingFromString(sCodePage);
